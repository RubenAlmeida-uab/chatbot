...
class BotController:
    ...
    async def gerar_relatorio(self, admin_id=None):
        """
        Obtém as estatísticas do modelo e usa a view para gerar um relatório em PDF.

        Args:
            admin_id (int, optional): ID do administrador que solicitou o relatório.
                                    Se fornecido, notifica os listeners.

        Returns:
            discord.File: Arquivo PDF do relatório
        """
        try:
            # Obtém as estatísticas usando o consulta_model
            estatisticas = self.consulta_model.obter_estatisticas()

            # Notifica os listeners se um admin_id foi fornecido
            if admin_id:
                self._notificar_relatorio_gerado(admin_id, "relatorio", "")

            # Usa a view para gerar o relatório em PDF
            return self.view.gerar_relatorio_pdf(estatisticas)
